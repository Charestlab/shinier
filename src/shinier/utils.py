...
def show_processing_overview(processor: ImageProcessor, img_idx: int = 0, show_figure: bool = True, show_initial_target: bool = False) -> plt.Figure:
    ...
    # --- Figure layout ---
    fft_padding = processor.options.fft_padding_mode != 0
    diag_steps = []
    for step in steps:
        if step == "dithering":
            continue
    ...
    for i, step in enumerate(diag_steps, start=1):
        ...
        # ---- Spatial frequency matching ----
            ...
            mag_before, _ = image_spectrum(
                processor._initial_buffer[img_idx],
                fft_padding_mode=processor.options.fft_padding_mode if padded_row else 0,
                fft_padding_value=processor.options.fft_padding_value if padded_row else 300,
            )
